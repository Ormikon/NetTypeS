...
namespace NetTypeS
{
	...
	{
		private readonly Lazy<IDictionary<Type, Type[]>> typeInheritance;

		public InheritedTypeSpy(params Assembly[] assemblies)
		{
			typeInheritance = new Lazy<IDictionary<Type, Type[]>>(() => InitializeTypes(assemblies));
		}
		...
		{
			Type[] types;
			return type == null || !typeInheritance.Value.TryGetValue(type, out types) ? Enumerable.Empty<Type>() : types;
		}
	}
}
